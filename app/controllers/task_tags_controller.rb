class TaskTagsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @task_tag.update(task_tag_params)
        format.html { redirect_to @task_tag, notice: "Task tag was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to task_tags_path, notice: "Task tag was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def task_tag_params
      params.expect(task_tag: [ :task_id, :tag_name ])
    end
end
