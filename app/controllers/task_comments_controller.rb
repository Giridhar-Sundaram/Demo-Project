class TaskCommentsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @task_comment.update(task_comment_params)
        format.html { redirect_to @task_comment, notice: "Task comment was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to task_comments_path, notice: "Task comment was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_task_comment
      @task_comment = TaskComment.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def task_comment_params
      params.expect(task_comment: [ :task_id, :author_id, :text, :date ])
    end
end
